...
// GET - Get treatments for a patient
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(patientTreatments);
  } catch (error) {
...
export async function POST(
  ...
  try {
    ...
    await logAudit(session.user.id, 'CREATE', 'TREATMENT', newTreatment[0].id, {
      patientId: params.patientId,
      assessmentId: assessment[0].id,
    });
    
    return NextResponse.json(newTreatment[0], { status: 201 });
  } catch (error) {
