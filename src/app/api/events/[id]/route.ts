...
export async function GET(
  ...
  try {
    ...
    await logAudit(session.user.id, 'READ', 'EVENT', params.id);
...
export async function PATCH(
  ...
  try {
    ...
    const result = await db.execute<EventRecord>(
      sql`UPDATE events SET ${setClause} WHERE id = ${params.id} RETURNING *`
    );
    ...
    await logAudit(session.user.id, 'UPDATE', 'EVENT', params.id);
...
export async function DELETE(
  ...
  try {
    const result = await db.execute<EventRecord>(
      sql`DELETE FROM events WHERE id = ${params.id} RETURNING *`
    );
    ...
    await logAudit(session.user.id, 'DELETE', 'EVENT', params.id);
